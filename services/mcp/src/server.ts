...
class MCPServer {
  ...
  private setupMiddleware(): void {
    ...
    // Body parsing
    this.app.use(express.json({ limit: '100mb' })); // Large limit for n8n workflows
    ...
    // Rate limiting
    this.app.use(AuthMiddleware.rateLimit(30, 60000)); // 30 requests per minute

    // Input sanitization
    this.app.use(ValidationMiddleware.sanitizeInput());

    this.logger.info('MCP service middleware setup completed');
  }
  ...
  private setupRoutes(): void {
    ...
    this.app.use('/api/v1',
      ...
      ValidationMiddleware.validatePayload('mcp'),
    ...
    this.app.get('/', (req, res) => {
      res.json({
        service: 'Umbra MCP Service',
        version: '1.0.0',
        status: 'running',
        capabilities: [
          'n8n API abstraction layer',
    ...
    this.logger.info('MCP service routes setup completed');
  }
  ...
  private setupErrorHandling(): void {
    ...
    this.logger.info('MCP service error handling setup completed');
  }
  ...
  public async start(): Promise<void> {
    try {
      ...
      this.app.listen(this.port, () => {
        this.logger.info('MCP Service started', {
          port: this.port,
          environment: process.env.NODE_ENV || 'development',
          n8nEnabled: !!process.env.N8N_API_URL,
          timestamp: new Date().toISOString()
        });
      });

    } catch (error) {
      this.logger.error('Failed to start MCP service', { error: (error as Error).message });
      process.exit(1);
    }
  }
